...
# https://github.com/docker/docker/blob/9a9fc01af8fb5d98b8eec0740716226fadb3735c/contrib/mkimage/debootstrap#L134-L151
	...
	&& linux32 apt-get update && linux32 apt-get install -y -q ca-certificates locales gcc wget curl software-properties-common \
		imagemagick libfreetype6 libgsf-bin libodbc1 socat python-pip zip unzip tar mono-runtime mono-mcs git-core \
		net-tools libmono-i18n4.0-all \
...
for v in "${versions[@]}"; do
	...
	( set -x; docker build -t "ubuntu32:$v" "$v" )
	serial="$(awk -F '=' '$1 == "SERIAL" { print $2; exit }' "$v/build-info.txt")"
	if [ "$serial" ]; then
		( set -x; docker tag "ubuntu32:$v" "ubuntu32:$v-$serial" )
	fi
	if [ -s "$v/alias" ]; then
		for a in $(< "$v/alias"); do
			( set -x; docker tag "ubuntu32:$v" "ubuntu32:$a" )
		done
	fi
	if [ "$v" = "$latest" ]; then
		( set -x; docker tag "${user}ubuntu32:$v" "ubuntu32:latest" )
	fi
done
